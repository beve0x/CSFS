function [W, elapse] = constructWr(fea,kk,options)
%	Usage:
    ...
    switch lower(options.WeightMode)
        case {lower('Binary')}
            if options.k > 0
                ...
                for i=1:nLabel
                    classIdx = find(options.gnd==Label(i));
                    classIdx2=find(options.gnd~=Label(i));
                    D = EuDist2(fea(classIdx,:),fea(classIdx2,:),0);
                    [dump idx] = sort(D,2);
                    idx = idx(:,1:options.k+1);
                    for k=1:length(classIdx)
                        G(classIdx(k),classIdx2(idx(k,:))) = 1;
                    end
                end
            end
        ...
        case {lower('HeatKernel')}
            if options.k > 0
                ...
                for i=1:nLabel
                    ...
                    dump = exp(-dump/(2*options.t^2));
                    for k=1:length(classIdx)
                        G(classIdx(k),classIdx2(idx(k,:))) = dump(k,:);
                    end
                end
            end
